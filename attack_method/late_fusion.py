...
TOKEN = "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"

comet_ml.login(project_name=PROJECT_NAME,api_key="9dzht80X6Z1LHikPFwGXM9Fsb")
...
def encoder_fgsm(X, model, eps=16/255, step_size=0.015, iters=40, clamp_min=-1, clamp_max=1, experiment=None):
    ...
    original_logit = model(X_adv)
    original_target = original_logit.argmax(dim=1)
    if original_target == 0:
        loss = original_logit[0][0]
    else:
        loss = original_logit[0][1]
...
def joint_attack(path=None, eps=16/255, step_size=4/255, iters=200, mus=0.5, project_name=None):
    """Joint Attack."""
    login(token="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")
    ...
    for i in tqdm(range(len(SEEDS))):
        ...
        with torch.autocast('cuda'):
            ...
            adv_X_encoder = encoder_pgd_inf(X,
            ...
            adv_X_resnet = encoder_fgsm(X, model, eps=eps)
                
        
            adv_X = mu * adv_X_encoder + (1 - mu) * adv_X_resnet
...
if __name__ == '__main__':
    ...
    parser.add_argument("--mu", type=float,
                        default=0.5,
                        help="perturbation weighting hyperparameters.")
